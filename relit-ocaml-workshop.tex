...
Consequently, we first typecheck against this overly permissive typing context, which ensures that there are at least no dependencies on locally-defined variables, types, or modules. Then, we write the parse tree to a temporary file, and run \li{ocamldep -modules} against the file. This gives us the list of free modules, which we check explicitly against the dependencies, resulting in a completely context independent proto-expansion.
...
\begin{lstlisting}
  ((fun a b -> (* ... proto-expansion with `a()`, `b()` instead of splices ... *)) 
...
We think TLMs would be useful when using ``normal'' OCaml syntax too. The main issue here is that every TLM must then support both syntaxes within splices. To achieve this, we plan to use Menhir's parameterization support to pass in syntax-agnostic versions of the helper functions. Also, given the recent news that the OCaml parser will be rewritten using Menhir, it makes sense for that effort to be completed before porting Relit.

There are also several remaining questions that have to do with integration with other components of OCaml's tooling. Although Merlin has basic support for TLMs, we need to add support for communicating the segmentation so that the editor can highlight spliced segments, and we would like for the programmer to be able to request the type of the spliced segment under the cursor. Another difficulty has to do with \li{ocamldep}, which cannot see inside spliced expressions in a context-free manner to extract dependencies. We plan to use the recently added support for \li{ppx} extensions in \li{ocamldep} to eliminate this restriction, but this is non-trivial due to the need to be able to typecheck the unexpanded program. Some build systems, notably \li{jbuilder}, currently limit \li{ppx} extensions from being able to access compiler flags in a manner that causes problems for Relit. We currently only test with \li{rebuild}/\li{ocamlbuild}.

Overall, we believe that our experiences developing Relit, which in many ways ``stress tested'' the PPX system, will be of interest to others working to improve OCaml's tooling, and we believe that Relit itself has the potential to be a valuable new mechanism for the OCaml community. 
