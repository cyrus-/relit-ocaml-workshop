...
\end{lstlisting}
We replace each encoded splice with a fresh variable applied to the unit value, construct a lambda that abstracts over those variables, and immediately apply this lambda to the corresponding spliced expressions, themselves ``thunked'' Kunder lambdas to maintain the ordering of side effects. 
